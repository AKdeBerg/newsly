...
export default function US({ usNewsProp }) {
  const [searchField, setSearchField] = useState('');
  const [pageNumber, setPageNumber] = useState(0);
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(actions.loadUSDataSuccess(usNewsProp.slice(0, 48)));
  }, []);

  const usNews = usNewsProp.slice(0, 48);

  // pagination
  const currentPageFirstArticleNumber = pageNumber * ARTICLE_PER_PAGE;
  const totalPages = calculateTotalPages(searchField, usNews, currentPageFirstArticleNumber);
      ...
      <div className="container">
        <div className="row">
          <div className="col-12">
            <h2 className="text-uppercase border-bottom mb-4 primary text-light">Top US News</h2>
...
export async function getStaticProps() {
  const res = await api.getUSTopStories();
  const usNews = res.data.results;
